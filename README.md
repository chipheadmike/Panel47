...
The unit tests cover the calculator engine, session management, settings persistence, the stardate math, the process list, the listening-port scan and their terminate safeguards, the ping and route parsers, the security checks, the Time Machine backup-age parser, the folder-size scanner, the git status parser, Homebrew detection, output parsing, the background command runner, and the password-prompt detector. The UI test target is only a launch check and needs macOS UI automation permission to run.
